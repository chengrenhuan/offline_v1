...
select
    ...
    tm_id,
    tm_name,
    sum(if(dt>=date_add('20250916',-6),order_count_1d,0)),
    sum(if(dt>=date_add('20250916',-6),order_num_1d,0)),
    sum(if(dt>=date_add('20250916',-6),order_original_amount_1d,0)),
    sum(if(dt>=date_add('20250916',-6),activity_reduce_amount_1d,0)),
    sum(if(dt>=date_add('20250916',-6),coupon_reduce_amount_1d,0)),
    sum(if(dt>=date_add('20250916',-6),order_total_amount_1d,0)),
    sum(order_count_1d),
    sum(order_num_1d),
...
insert overwrite table dws_trade_province_order_nd partition(dt='20250916')
select
    province_id,
    province_name,
    area_code,
...
insert overwrite table dws_trade_user_order_td partition(dt='20250916')
select
    user_id,
...
select nvl(old.user_id, new.user_id),
       if(old.user_id is not null, old.order_date_first, '20250916'),
       if(new.user_id is not null, '20250916', old.order_date_last),
...
from (
         ...
         union all
         select user_id,
                '20250916' order_date_first,
                '20250916' order_date_last,
                order_count_1d,
                order_num_1d,
                order_original_amount_1d,
                activity_reduce_amount_1d,
